        ...
        let params = {
            g: 9.81, rho: 1.225, mass: 1.0, area: 0.01, cd: 0.47,
            type: 'lead', theme: themes.earth
        };
        ...
        function updateEnvironment() {
            ...
            else if (env === 'jupiter') { params.g = 24.79; params.rho = 0.5; } // Higher rho for Jupiter atmosphere
        ...
        function createImpact(x, velocity) {
            const count = Math.min(Math.abs(velocity) * 2, 40);
            for (let i = 0; i < count; i++) {
                particles.push({
                    x: x,
                    y: canvas.height - 50,
                    vx: (Math.random() - 0.5) * velocity * 0.5,
                    vy: -Math.random() * velocity * 0.3,
                    life: 1.0,
        ...
        function drawBackground() {
            ...
            ctx.lineWidth = 4;
            ctx.strokeRect(-10, canvas.height - 50, canvas.width + 20, 5);
        }
        ...
        function drawFeather(x, y, velocity) {
            ctx.save();
            ctx.translate(x, y);
            // Tilt the feather based on velocity or just a slight angle
        ...
        function calculatePhysics(obj, dt) {
            ...
            const groundY = canvas.height - 65; // Adjust for object size
            if (obj.y >= groundY) {
        ...
        function render(time = 0) {
            ...
            // Stats Update
            statTime.textContent = objectA.t.toFixed(2);
            statVel.textContent = Math.abs(objectA.v).toFixed(2);
            statAcc.textContent = objectA.a.toFixed(2);
            statHeight.textContent = Math.max(0, (canvas.height - 65 - objectA.y) / PIXELS_PER_METER).toFixed(2);
