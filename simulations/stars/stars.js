        ...
        function init() {
            ...
            document.getElementById('massInput').addEventListener('input', (e) => {
                ...
                if(currentStageIndex === 5 || currentStageIndex === 2 || currentStageIndex === 1 || currentStageIndex === 3) {
            ...
            document.getElementById('wormholeBtn').addEventListener('click', () => {
                ...
                // עדכון הכותרת כך שתציג את השם המדעי וחור תולעת בסוגריים
        ...
        function checkWormholeButton() {
            const btn = document.getElementById('wormholeBtn');
            btn.style.display = (currentStageIndex === 5 && stellarMass >= 20) ? 'block' : 'none';
        }
        ...
        function updateStage(index) {
            ...
            switch(stage.id) {
                ...
                case 'protostar': 
                    createNebula(0xfb923c, 6, 1500); 
                    createStar(1.8, 0xff7722, true, true);
                    createAccretionDisk(3.5, 0xffaa44, 0.4, true);
                    createInfallingMatter(0xffaa44, 4);
                    break;
                case 'main_sequence':
                    const color = stellarMass < 2 ? 0xfde047 : (stellarMass < 10 ? 0xffffff : 0x60a5fa);
                    const size = 1 + (stellarMass * 0.1);
                    createStar(size, color, false, false, true);
                    createGlow(size * 1.5, color, 0.4);
                    createGlow(size * 2.2, color, 0.15);
                    createCoronalLoops(size, color);
                    break;
                case 'red_giant':
                    const rColor = stellarMass < 8 ? 0xef4444 : 0x991b1b;
                    const rSize = stellarMass < 8 ? 4 : 6;
                    createStar(rSize, rColor, false, false, false, true); 
                    stageGlow = createGlow(rSize * 1.3, rColor, 0.5);
                    break;
                case 'death':
                    if (stellarMass < 8) { createNebula(0x2dd4bf, 8, 3000, true); createStar(0.6, 0xffffff, false); }
                    else createExplosion();
                    break;
                case 'remnant':
                    if (stellarMass < 8) { createStar(0.4, 0xbfdbfe, false); createNebula(0x60a5fa, 6, 1000, false); createGlow(1.2, 0x60a5fa, 0.2); }
                    else if (stellarMass < 20) { createStar(0.3, 0xffffff, false); createJets(0x60a5fa, 20); starObject.userData = { isPulsar: true }; }
                    else createBlackHole();
                    break;
            }
        }
        ...
        function createStar(radius, color, isLowPoly, isProto = false, isMain = false, isGiant = false) {
            let mat;
            if (isMain) {
                const tex = generateStarTexture(new THREE.Color(color).getStyle(), 'granulated');
                mat = new THREE.MeshStandardMaterial({ map: tex, emissive: color, emissiveIntensity: 1.2 });
            } else if (isGiant) {
                const tex = generateStarTexture(new THREE.Color(color).getStyle(), 'plasma');
                mat = new THREE.MeshStandardMaterial({ map: tex, emissive: color, emissiveIntensity: 1.0 });
            } else {
                mat = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: isProto ? 2.5 : 0.6 });
            }
            const detail = isMain || isGiant ? 4 : (isLowPoly ? 1 : 2);
            starObject = new THREE.Mesh(new THREE.IcosahedronGeometry(radius, detail), mat);
            if (isGiant) {
                starObject.userData.origPos = starObject.geometry.attributes.position.array.slice();
                starObject.userData.isGiant = true;
                starObject.userData.radius = radius;
            }
        ...
        function createAccretionDisk(radius, color, opacity, layered = false) {
            ...
            for(let i=0; i<(layered ? 3 : 1); i++) {
                ...
                d.rotation.x = Math.PI / 2 + (Math.random() * 0.1);
                d.userData = { speed: 0.02 + (i * 0.01) };
                accretionDisk.add(d);
            }
        ...
        function createInfallingMatter(color, radius) {
            ...
            particleSystem = new THREE.Points(geo, new THREE.PointsMaterial({ color, size: 0.05, transparent: true, opacity: 0.8 }));
            particleSystem.userData = { type: 'infall', radius };
        ...
        function createNebula(color, radius, count, exp = false) {
            ...
            for (let i = 0; i < count; i++) {
                const r = radius * Math.pow(Math.random(), exp ? 2 : 0.6);
            ...
            nebulaSystem = new THREE.Points(geo, new THREE.PointsMaterial({ color, size: 0.1, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending }));
        ...
        function createExplosion() {
            ...
            geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
            particleSystem = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0xffaa00, size: 0.15, transparent: true, blending: THREE.AdditiveBlending }));
            particleSystem.userData = { time: 0, dirs, type: 'explosion' }; rotationGroup.add(particleSystem);
        }

        function createBlackHole() {
            starObject = new THREE.Mesh(new THREE.SphereGeometry(1, 32, 32), new THREE.MeshBasicMaterial({ color: 0x000000 }));
            rotationGroup.add(starObject);
            accretionDisk = new THREE.Mesh(new THREE.TorusGeometry(2.5, 0.2, 2, 50), new THREE.MeshBasicMaterial({ color: 0xff7700, wireframe: true }));
            accretionDisk.rotation.x = Math.PI / 2.5; rotationGroup.add(accretionDisk);
        }

        function createJets(color, length) {
            const jetGeo = new THREE.CylinderGeometry(0.1, 0.8, length, 12, 1, true);
            ...
            const j1 = new THREE.Mesh(jetGeo, jetMat); const j2 = new THREE.Mesh(jetGeo, jetMat);
            j1.position.y = length/2; j2.position.y = -length/2; j2.rotation.z = Math.PI;
            jets.push(j1, j2); rotationGroup.add(j1, j2);
        }

        function createCoronalLoops(size, color) {
            for(let i=0; i<8; i++) {
                const curve = new THREE.EllipseCurve(0, 0, size*0.4, size*0.6, 0, Math.PI, false, 0);
                const points = curve.getPoints(20);
                const geometry = new THREE.BufferGeometry().setFromPoints(points);
                const loop = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.5 }));
                loop.rotation.x = Math.random() * Math.PI; loop.rotation.y = Math.random() * Math.PI;
                loop.position.set(Math.random()-0.5, Math.random()-0.5, Math.random()-0.5).normalize().multiplyScalar(size*0.8);
                loop.userData = { phase: Math.random() * Math.PI * 2 };
                coronalLoops.push(loop); rotationGroup.add(loop);
            }
        }

        function createForwardTunnel() {
            const canvas = document.createElement('canvas'); canvas.width = 512; canvas.height = 512;
            const ctx = canvas.getContext('2d'); ctx.fillStyle = '#050508'; ctx.fillRect(0, 0, 512, 512);
            ...
            const tunnelMat = new THREE.MeshBasicMaterial({ map: texture, side: THREE.BackSide, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending });
            wormhole = new THREE.Mesh(new THREE.CylinderGeometry(2, 10, 100, 32, 20, true), tunnelMat);
            wormhole.rotation.x = Math.PI / 2; rotationGroup.add(wormhole);
            rotationGroup.add(new THREE.Mesh(new THREE.SphereGeometry(1, 16, 16), new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 })));
        }
        ...
        function animate() {
            ...
            if (nebulaSystem) {
                nebulaSystem.rotation.y += 0.001;
                if (nebulaSystem.userData.exp) {
                    const pos = nebulaSystem.geometry.attributes.position.array;
                    for(let i=0; i<pos.length; i++) pos[i] *= 1.008;
                    nebulaSystem.geometry.attributes.position.needsUpdate = true;
                    nebulaSystem.material.opacity *= 0.995;
                }
            }
            if (starObject) {
                starObject.rotation.y += starObject.userData.isPulsar ? 0.3 : 0.005;
                if (starObject.userData.isPulsar) starObject.rotation.z += 0.05;
                if (currentStageIndex === 2) starObject.scale.setScalar(1 + Math.sin(time * 2) * 0.01);
                if (starObject.userData.isGiant) {
                    const position = starObject.geometry.attributes.position;
                    const array = position.array;
                    const orig = starObject.userData.origPos;
                    for (let i = 0; i < array.length; i += 3) {
                        ...
                        const factor = 1.0 + noise * 0.12;
                        array[i] = vx * factor; array[i+1] = vy * factor; array[i+2] = vz * factor;
                    }
                    position.needsUpdate = true;
                }
            }
            if (stageGlow && currentStageIndex === 3) {
                stageGlow.scale.setScalar(1 + Math.sin(time * 0.5) * 0.05);
                stageGlow.material.opacity = 0.4 + Math.sin(time * 0.8) * 0.1;
            }
            if (particleSystem) {
                const pos = particleSystem.geometry.attributes.position.array;
                if (particleSystem.userData.type === 'infall') {
                    for(let i=0; i<pos.length/3; i++) {
                        const v = new THREE.Vector3(pos[i*3], pos[i*3+1], pos[i*3+2]);
                        v.multiplyScalar(0.98); 
                        if (v.length() < 1.5) v.setLength(5 + Math.random()*2); 
                        pos[i*3] = v.x; pos[i*3+1] = v.y; pos[i*3+2] = v.z;
                    }
                } else {
                    const dirs = particleSystem.userData.dirs; particleSystem.userData.time += delta;
                    for(let i=0; i<dirs.length; i++) {
                        pos[i*3] += dirs[i].x * delta; pos[i*3+1] += dirs[i].y * delta; pos[i*3+2] += dirs[i].z * delta;
                    }
                    particleSystem.material.opacity = Math.max(0, 1 - particleSystem.userData.time / 4);
                }
                particleSystem.geometry.attributes.position.needsUpdate = true;
            }
            if (accretionDisk && accretionDisk.children) {
                accretionDisk.children.forEach(d => { d.rotation.z += d.userData.speed || 0.02; });
            } else if (accretionDisk) accretionDisk.rotation.z += 0.02;
            coronalLoops.forEach(loop => {
                const s = 1 + Math.sin(time * 3 + loop.userData.phase) * 0.1;
                loop.scale.set(s, s, s);
            });
            if (wormhole && wormhole.material.map) {
                wormhole.material.map.offset.y -= delta * 2.0;
                camera.position.x = Math.sin(time * 20) * 0.02; camera.position.y = Math.cos(time * 20) * 0.02;
            } else { camera.position.x = 0; camera.position.y = 0; }
            jets.forEach(jet => {
                jet.scale.x = 0.8 + Math.sin(time * 10) * 0.2;
                jet.material.opacity = 0.3 + Math.sin(time * 15) * 0.2;
            });
